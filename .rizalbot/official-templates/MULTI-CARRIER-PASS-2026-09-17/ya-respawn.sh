...
# macOS ships bash 3.2 — no mapfile; read two lines from pick_template
PICK_OUT="$(pick_template "$REQUESTED_ID")"
TID="$(printf '%s
' "$PICK_OUT" | sed -n '1p')"
TPATH="$(printf '%s
' "$PICK_OUT" | sed -n '2p')"
...
if command -v rsync >/dev/null 2>&1; then
  rsync -a --delete \
    ...
    --exclude build \
    --exclude '*.xcuserstate' \
    --exclude 'engine/heart.gguf' \
    --exclude 'heart.gguf' \
    "$SRC/" "$LIVE/"
  # HARDCODE: never wipe local Heart GGUF when template ships HEART-SEAT only
  if [[ ! -f "$LIVE/engine/heart.gguf" ]]; then
    for cand in \
      "${YBOT_ROOT}/hearts-archive/heart-chat-qwen25-3b-q4.gguf" \
      "${YBOT_ROOT}/ЯBOT/engine/heart.gguf"
    do
      if [[ -f "$cand" ]]; then
        mkdir -p "$LIVE/engine"
        cp -c "$cand" "$LIVE/engine/heart.gguf" 2>/dev/null || cp "$cand" "$LIVE/engine/heart.gguf"
        echo "    heart: restored engine/heart.gguf ← $cand"
        break
      fi
    done
  else
    echo "    heart: kept existing engine/heart.gguf"
  fi
else
