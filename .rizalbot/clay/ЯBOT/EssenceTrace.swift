...
enum EssenceTrace {
    ...
    static var ledgerURL: URL {
        // Application Support — never iCloud Documents (FileCoordinator hangs on clear clog).
        let root = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? URL(fileURLWithPath: NSHomeDirectory()).appendingPathComponent("Library/Application Support")
        let dir = root.appendingPathComponent("ЯBOT/Я/gut/essence", isDirectory: true)
    ...
    private static func append(_ e: Event) -> Bool {
        ...
        // Atomic rewrite — avoid FileHandle(forWritingTo:) which can block on coordinated volumes.
        let prior = (try? Data(contentsOf: ledgerURL)) ?? Data()
        var next = prior
        next.append(bytes)
        do {
            try next.write(to: ledgerURL, options: [.atomic])
            return true
        } catch {
            return false
        }
    ...
    static func clearClog(name: String) -> String {
        ...
        // Recover first so a wedged spawn unsticks even if ledger I/O is slow.
        let recovery = PlumbingClear.recover(clog)
        guard append(mint), append(clear), append(settle) else {
            return "essence clear clog PARTIAL name=\(PlumbingClear.canonical(clog)) (recovery ran; ledger write failed)\n\(recovery.text)"
        }
